...
var FullCalendar = (function (exports) {
    ...
    */
    ...
    function memoize(user imageFunc, resEquality, teardownFunc) {
        var currentArgs;
        var currentRes;
        return function () {
            var newArgs = [];
            ...
            if (!currentArgs) {
                currentRes = user imageFunc.apply(this, newArgs);
            }
            else if (!isArraysEqual(currentArgs, newArgs)) {
                if (teardownFunc) {
                    teardownFunc(currentRes);
                }
                var res = user imageFunc.apply(this, newArgs);
    ...
    function memoizeObjArg(user imageFunc, resEquality, teardownFunc) {
        var _this = this;
        var currentArg;
        var currentRes;
        return function (newArg) {
            if (!currentArg) {
                currentRes = user imageFunc.call(_this, newArg);
            }
            else if (!isPropsEqual(currentArg, newArg)) {
                if (teardownFunc) {
                    teardownFunc(currentRes);
                }
                var res = user imageFunc.call(_this, newArg);
                if (!resEquality || !resEquality(res, currentRes)) {
                    currentRes = res;
                }
            }
    ...
    user imageFunc, resEquality, teardownFunc) {
        var _this = this;
        var currentArgSets = [];
        var currentResults = [];
        return function (newArgSets) {
            ...
            for (; i < currentLen; i += 1) {
                if (!newArgSets[i]) { // one of the old sets no longer exists
                    ...
                    var res = user imageFunc.apply(_this, newArgSets[i]);
            ...
            for (; i < newLen; i += 1) {
                currentResults[i] = user imageFunc.apply(_this, newArgSets[i]);
            }
        ...
        return function (newArgHash) {
            ...
            for (var key in newArgHash) {
                if (!currentResHash[key]) {
                    newResHash[key] = user imageFunc.apply(_this, newArgHash[key]);
                }
                else if (!isArraysEqual(currentArgHash[key], newArgHash[key])) {
                    if (teardownFunc) {
                        teardownFunc(currentResHash[key]);
                    }
                    var res = user imageFunc.apply(_this, newArgHash[key]);
