      ...
      return this.ui.buttonGroup({
        ...
        children: [this.button({
          ...
          callback: function callback($button) {
            ...
            if (backColor) {
              $recentColor.css('', _this.options.colorButton.backColor);
          ...
          callback: function callback($dropdown) {
            ...
            $dropdown.find('input[type=color]').each(function (idx, item) {
              external_jQuery_default()(item).change(function () {
                ...
                $chip.css('', color).attr('aria-label', color).attr('data-value', color).attr('data-original-title', color);
                $chip.click();
              });
            });
          },
          click: function click(event) {
            ...
            if (eventName === 'openPalette') {
              ...
              if (lists.contains(['backColor', 'foreColor'], eventName)) {
                var key = eventName === 'backColor' ? '' : 'color';
...
var palette = renderer.create('<div class="note-color-palette"></div>', function ($node, options) {
  ...
  for (var row = 0, rowSize = options.colors.length; row < rowSize; row++) {
    ...
    for (var col = 0, colSize = colors.length; col < colSize; col++) {
      ...
      buttons.push(['<button type="button" class="note-btn note-color-btn"', 'style=":', color, '" ', 'data-event="', eventName, '" ', 'data-value="', color, '" ', 'data-title="', colorName, '" ', 'aria-label="', colorName, '" ', 'data-toggle="button" tabindex="-1"></button>'].join(''));
    }
